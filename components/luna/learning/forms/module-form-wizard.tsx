...
/**
 * Module Form Wizard Component
 * Multi-step form for creating/editing modules
...
import { ModuleContentSelector, type ContentItem } from '@/components/luna/learning/module-content-selector';
...
export interface ModuleFormData {
  ...
  requirements: string[];
  skills: string[];
  content: ContentItem[]; // Combined lessons and quizzes
  scholarships: string[]; // scholarship IDs
}

export interface ModuleFormWizardProps {
  initialData?: Module;
...
export interface ModuleFormWizardRef {
  currentStep: number;
  totalSteps: number;
  isStepValid: () => boolean;
  handleNext: () => void;
...
export const ModuleFormWizard = forwardRef<ModuleFormWizardRef, ModuleFormWizardProps>(function ModuleFormWizard({
  initialData,
  initialContent = [],
  initialScholarships = [],
  onSubmit,
  ...
  // Reset form data when initialData.id or initialContent length changes (indicates new module being edited)
  useEffect(() => {
    setFormData({
      title: initialData?.title || '',
      description: initialData?.description || '',
    ...
    // Only re-run when the module ID changes or content/scholarships array length changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialData?.id, initialContent.length, initialScholarships.length]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch quizzes
        const quizzesRes = await fetch('/api/learning/quizzes', {
          credentials: 'include',
        });
        if (quizzesRes.ok) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="min-h-[300px]">
              ...
              {formData.scholarship_eligible && (
                <div className="col-span-2">
                  <LunaCombobox
                    ...
                    helperText="Select which scholarships can be applied to this module"
                  />
                </div>
              )}
            </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-xs text-luna-gray-600 mb-3">
                What will students learn from this module?
              </p>
            </div>
            <LunaMultiTextInput
        ...
        {currentStep === 4 && (
          <div className="space-y-3">
            <div>
              ...
              <p className="text-xs text-luna-gray-600 mb-3">
                What should students know before taking this module?
              </p>
            </div>
            <LunaMultiTextInput
        ...
        {/* Step 6: Content (Lessons & Quizzes) */}
        {currentStep === 5 && (
          <div className="space-y-3">
            <ModuleContentSelector
              ...
              onChange={(selectedContent) => setFormData({ ...formData, content: selectedContent })}
              helperText="Add lessons and quizzes in any order. Drag to reorder."
            />
          </div>
        )}
      </div>
