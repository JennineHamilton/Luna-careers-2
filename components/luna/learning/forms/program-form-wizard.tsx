...
/**
 * Program Form Wizard Component
 * Multi-step form for creating/editing programs
...
import { LunaCourseSelector, type SelectedCourse } from '@/components/luna/learning/course-selector';
...
export interface ProgramFormWizardProps {
  initialData?: Program;
...
export interface ProgramFormWizardRef {
  currentStep: number;
  totalSteps: number;
  isStepValid: () => boolean;
  handleNext: () => void;
...
export const ProgramFormWizard = forwardRef<ProgramFormWizardRef, ProgramFormWizardProps>(function ProgramFormWizard({
  initialData,
  initialCourses = [],
  initialScholarships = [],
  onSubmit,
  ...
  // Reset form data when initialData.id or initialCourses length changes (indicates new program being edited)
  useEffect(() => {
    setFormData({
      title: initialData?.title || '',
      description: initialData?.description || '',
    ...
    // Only re-run when the program ID changes or courses/scholarships array length changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialData?.id, initialCourses.length, initialScholarships.length]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch courses
        const coursesRes = await fetch('/api/learning/courses?published_only=false');
        if (coursesRes.ok) {
    ...
    <div className="space-y-4">
      ...
      <div className="min-h-[300px]">
              ...
              {formData.scholarship_eligible && (
                <div className="col-span-2">
                  <LunaCombobox
                    ...
                    helperText="Select which scholarships can be applied to this program"
                  />
                </div>
              )}
            </div>
        ...
        {currentStep === 5 && (
          <div className="space-y-3">
            <LunaCourseSelector
              availableCourses={courses.map(m => ({ id: m.id, title: m.title, duration_minutes: m.duration_minutes ?? undefined }))}
              selectedCourses={formData.courses}
              onChange={(selectedCourses) => setFormData({ ...formData, courses: selectedCourses })}
            />
          </div>
        )}
      </div>
