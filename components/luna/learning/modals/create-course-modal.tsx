...
export function CreateCourseModal({
  ...
  const handleSubmit = async (formData: CourseFormData) => {
    ...
    try {
      ...
      // Create course_modules relationships
      if (formData.modules.length > 0 && result.course) {
        ...
        if (modulesError) {
          ...
          setError('Course created but failed to add modules');
          setLoading(false);
          return;
        }
      }
      ...
      if (formData.scholarships && formData.scholarships.length > 0 && result.course) {
        const scholarshipContent = formData.scholarships.map((scholarshipId) => ({
          scholarship_id: scholarshipId,
          content_type: 'course',
          content_id: result.course.id,
        }));
    ...
    <LunaDialog open={open} onOpenChange={onOpenChange}>
      <LunaDialogContent className="max-w-4xl">
        ...
        <LunaDialogHeader>
          ...
          <LunaDialogDescription>
            Create a new learning course with modules and content
          </LunaDialogDescription>
        </LunaDialogHeader>
