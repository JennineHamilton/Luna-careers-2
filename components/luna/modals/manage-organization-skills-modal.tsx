/**
 * Manage Organization Skills Modal
 * Adapted from AddSkillsModal for organization_skills
 */

'use client';

import { useState, useEffect } from 'react';
import {
  LunaDialog,
  LunaDialogContent,
  LunaDialogHeader,
  LunaDialogTitle,
  LunaDialogDescription,
  LunaDialogBody,
  LunaDialogFooter,
  LunaButton,
  LunaCombobox,
  LunaBadge,
} from '@/components/luna';
import { Loader2, Lightbulb, X } from 'lucide-react';
import { createClient } from '@/lib/supabase/client';
import { useRouter } from 'next/navigation';
import type { Database } from '@/types/database.types';

type Skill = Database['public']['Tables']['skills']['Row'];
type OrganizationSkill = Database['public']['Tables']['organization_skills']['Row'] & {
  skills: Database['public']['Tables']['skills']['Row'];
};

interface ManageOrganizationSkillsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  organizationId: string;
  initialSkills: OrganizationSkill[];
  onSuccess?: () => void;
}

export function ManageOrganizationSkillsModal({
  open,
  onOpenChange,
  organizationId,
  initialSkills,
  onSuccess,
}: ManageOrganizationSkillsModalProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [allSkills, setAllSkills] = useState<Skill[]>([]);
  const [orgSkills, setOrgSkills] = useState<OrganizationSkill[]>(initialSkills);
  const [selectedSkillIds, setSelectedSkillIds] = useState<string[]>([]);

  useEffect(() => {
    if (open) {
      setOrgSkills(initialSkills);
      fetchAllSkills();
    }
  }, [open, initialSkills]);

  const fetchAllSkills = async () => {
    const supabase = createClient();

    // Fetch all available skills
    const { data: skillsData, error: skillsError } = await supabase
      .from('skills')
      .select('*')
      .order('name');

    if (skillsError) {
      console.error('Error fetching skills:', skillsError);
      setError('Failed to load skills');
    } else if (skillsData) {
      setAllSkills(skillsData);
    }
  };

  const handleSave = async () => {
    // If no new skills to add, just close the modal
    if (selectedSkillIds.length === 0) {
      onOpenChange(false);
      setSelectedSkillIds([]);
      return;
    }

    setError('');
    setLoading(true);

    try {
      const supabase = createClient();
      const { data: { user } } = await supabase.auth.getUser();

      if (!user) throw new Error('Not authenticated');

      // Insert new skills
      const { error: insertError } = await supabase
        .from('organization_skills')
        .insert(
          selectedSkillIds.map(skillId => ({
            organization_id: organizationId,
            skill_id: skillId,
          }))
        );

      if (insertError) throw insertError;

      setLoading(false);
      setSelectedSkillIds([]);
      onOpenChange(false);
      onSuccess?.();
      router.refresh();
    } catch (err) {
      console.error('Error adding skills:', err);
      setError('An error occurred. Please try again.');
      setLoading(false);
    }
  };

  const handleDeleteSkill = async (orgSkillId: string) => {
    try {
      const supabase = createClient();
      const { error: deleteError } = await supabase
        .from('organization_skills')
        .delete()
        .eq('id', orgSkillId);

      if (deleteError) throw deleteError;

      // Update local state
      setOrgSkills(orgSkills.filter(os => os.id !== orgSkillId));
      router.refresh();
    } catch (err) {
      console.error('Error deleting skill:', err);
      setError('Failed to delete skill');
    }
  };

  // Filter out skills the organization already has
  const availableSkills = allSkills.filter(
    skill => !orgSkills.some(os => os.skill_id === skill.id)
  );

  return (
    <LunaDialog open={open} onOpenChange={onOpenChange}>
      <LunaDialogContent className="max-w-2xl">
        <LunaDialogHeader>
          <LunaDialogTitle>
            <div className="flex items-center gap-2">
              <Lightbulb className="w-5 h-5" />
              Manage Required Skills
            </div>
          </LunaDialogTitle>
          <LunaDialogDescription>
            Add or remove skills required to work for your company
          </LunaDialogDescription>
        </LunaDialogHeader>

        <LunaDialogBody>
          <div className="space-y-6">
            {error && (
              <div className="p-3 bg-luna-red-50 border border-luna-red-200 rounded-lg text-sm text-luna-red-700">
                {error}
              </div>
            )}

            {/* Current Skills */}
            {orgSkills.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-luna-gray-900 mb-2">
                  Required Skills
                </label>
                <div className="flex flex-wrap gap-2">
                  {orgSkills.map((orgSkill) => (
                    <LunaBadge
                      key={orgSkill.id}
                      variant="default"
                      className="flex items-center gap-1.5 pl-3 pr-2 py-1.5"
                    >
                      <span>{orgSkill.skills.name}</span>
                      <button
                        type="button"
                        onClick={() => handleDeleteSkill(orgSkill.id)}
                        className="hover:bg-luna-gray-300 rounded-sm p-0.5 transition-colors"
                        aria-label="Remove skill"
                      >
                        <X className="w-3.5 h-3.5" />
                      </button>
                    </LunaBadge>
                  ))}
                </div>
              </div>
            )}

            {/* Add New Skills */}
            <div>
              <LunaCombobox
                label="Add Skills"
                options={availableSkills.map(skill => ({
                  value: skill.id,
                  label: skill.name,
                }))}
                value={selectedSkillIds}
                onChange={setSelectedSkillIds}
                placeholder="Select skills to add..."
                searchPlaceholder="Search skills..."
                helperText="Select one or more skills from the list"
              />
            </div>
          </div>
        </LunaDialogBody>

        <LunaDialogFooter>
          <LunaButton
            type="button"
            variant="secondary"
            onClick={() => {
              onOpenChange(false);
              setSelectedSkillIds([]);
            }}
            disabled={loading}
          >
            Cancel
          </LunaButton>
          <LunaButton
            onClick={handleSave}
            variant="primary"
            disabled={loading}
          >
            {loading && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
            {selectedSkillIds.length > 0
              ? `Add (${selectedSkillIds.length}) Skill${selectedSkillIds.length !== 1 ? 's' : ''}`
              : 'Done'
            }
          </LunaButton>
        </LunaDialogFooter>
      </LunaDialogContent>
    </LunaDialog>
  );
}
